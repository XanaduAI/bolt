...
def remove(pattern: tuple) -> Generator[Tuple[int, Tuple[int, ...]], None, None]:
...
def build_order(kbuild: Tuple[int], num_modes) -> Generator[Tuple[Tuple[int, ...], Tuple[int, ...], int], None, None]:
...
@jit(cache=True, nopython=True)
def add_photon_to_output(kin: Tuple[int], koutplus1: int, i: int, O: np.array, U: NumbaDict, dU: NumbaDict, grad: bool):
    """
    ...
    Arguments:
        ...
        O: current orthogonal covariance matrix
        U: current U_k^out dict
        dU current dU_k^out dict
    """
    _U = 0.0 + 0.0j
    for p, kin_p in remove(kin):
        _U += sqrt[kin[p]] * O[i, p] * U[kin_p]
    _U /= sqrt[koutplus1]
    _dU = None
    if grad:
        _dU = np.zeros_like(O)
        for p, kin_p in remove(kin):
            _dU += sqrt[kin[p]] * O[i, p] * dU[kin_p]
...
@jit(cache=True, nopython=True)
def add_photon_to_input(kout: Tuple[int], kinplus1: Tuple[int], i: int, O: np.ndarray, U: NumbaDict, dU: NumbaDict, grad: bool) -> Tuple[complex, np.ndarray]:
    """
    ...
    for p, kout_p in remove(kout):
        _U += sqrt[kout[p]] * O[p, i] * U[kout_p]
    _U /= sqrt[kinplus1]
    _dU = None
    if grad:
        _dU = np.zeros_like(O)
        for p, kout_p in remove(kout):
            _dU += sqrt[kout[p]] * O[p, i] * dU[kout_p]
...
# @jit(cache=True, nopython=True)
def all_outputs(state_in, O: np.ndarray, grad: bool = True) -> Tuple[Dict, Dict]:
    """
    Returns all the outputs of an interferometer O, given an input state_in.
    """
    num_modes = state_in.num_modes

    U: defaultdict = defaultdict(lambda: NumbaDict.empty(key_type=UniTuple(int64, num_modes), value_type=complex128))
    U[(0,) * num_modes][(0,) * num_modes] = 1.0 + 0.0j
    dU: defaultdict = defaultdict(lambda: NumbaDict.empty(key_type=UniTuple(int64, num_modes), value_type=complex128[:, :]))
    dU[(0,) * num_modes][(0,) * num_modes] = np.zeros_like(O, dtype=np.complex128)
        ...
        for prev_kbuild, current_kbuild, mode in build_order(kin, num_modes):
            ...
            for _kscan in partition(photons, (photons,) * num_modes):
                if _kscan not in U[current_kbuild]:
                    u, du = add_photon_to_output(
                        _kscan, current_kbuild[mode], mode, O, U[prev_kbuild], dU[prev_kbuild], grad
                    )
                    U[current_kbuild][_kscan] = u
                    if grad:
                        dU[current_kbuild][_kscan] = du

    out: defaultdict = defaultdict(complex)
    dout: defaultdict = defaultdict(complex)
