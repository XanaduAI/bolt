...
from .utils import partition, build_order, add_photon_to_output, add_photon_to_input
...
class Tree:
    ...
    def __init__(self, io: IOSpec, unitary_matrix: np.array = None, grad: bool = True):
        if unitary_matrix is None:
            modes = len(list(io.input.keys())[0])
            lambdas = np.random.normal(size=modes ** 2, scale=1)
            unitary_matrix = expm(L(lambdas))
        self.V = unitary_matrix
        ...
        for prev_kbuild, current_kbuild, mode in build_order(kbuild, self.num_modes):
            ...
            for _kscan in partition(photons, kscan):
                if _kscan not in self.U[current_kbuild]:  # not in dU[current_kbuild] either
                    ...
                    if self.grad:
                        self.dU[current_kbuild][_kscan] = dU

    def step(self, tup):
        kbuild, kscan = tup
        return self.build_tree(kbuild, kscan, self.io.building_input)
