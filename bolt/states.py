...
class IOSpec:
    ...
    def __init__(self, input_state: State, output_state: State, build: str = None):
        if len(set([sum(ket) for ket in input_state.keys()])) > 1:
            raise ValueError(
                f"""The input state spans more than one multiplet (photons per ket are {[sum(ket) for ket in input_state.keys()]}).
        ...
        if len(set([sum(ket) for ket in output_state.keys()])) > 1:
            raise ValueError(
                f"""The output state spans more than one multiplet (photons per ket are {[sum(ket) for ket in output_state.keys()]}).
        Solution: separate the required input-output relations per multiplet and submit them as different requirements."""
            )
        if sum(list(input_state.keys())[0]) != sum(list(output_state.keys())[0]):
            raise ValueError(
                f"""This spec does not conserve photon number
        ...
        if len(set([len(ket) for ket in input_state.keys()])) > 1:
            raise ValueError(
                f"not all input states span the same number of modes (modes = {[len(ket) for ket in input_state.keys()]})"
            )
        if len(set([len(ket) for ket in output_state.keys()])) > 1:
            raise ValueError(
                f"not all output states span the same number of modes (modes = {[len(ket) for ket in output_state.keys()]})"
            )
        self.input = input_state
        self.output = output_state
        if build is None:
            cost_of_building_input = approx_tree_cost(
                build_patterns=list(self.input.keys()), scan_patterns=list(self.output.keys())
            )
            cost_of_building_output = approx_tree_cost(
                build_patterns=list(self.output.keys()), scan_patterns=list(self.input.keys())
            )
        ...
        if "input" == build.lower():
            ...
            self.paths = [
                (ket_in, ket_out, self.input[ket_in] * np.conj(self.output[ket_out]))
                for ket_out in output_state
                for ket_in in input_state
            ]
        elif "output" in build.lower():
            self.building_output = True
            self.building_input = False
            self.paths = [
                (ket_out, ket_in, self.input[ket_in] * np.conj(self.output[ket_out]))
                for ket_out in output_state
                for ket_in in input_state
            ]
        else:
...
class Requirements:
    ...
    def __init__(self, specs: Dict[IOSpec, float]):
        if len(set([io.modes for io in specs.keys()])) > 1:
            raise ValueError(
                f"not all input-output relations span the same number of modes (modes = {[io.modes for io in specs.keys()]})"
            )
        self.specs = specs
